...
class StageCommandServiceIntegrationTest : ApplicationIntegrationTest() {
    ...
    fun setUp() {
        ...
        에픽하이_식별자 = artistRepository.save(ArtistFixture.builder().name("에픽하이").build()).id
        소녀시대_식별자 = artistRepository.save(ArtistFixture.builder().name("소녀시대").build()).id
        뉴진스_식별자 = artistRepository.save(ArtistFixture.builder().name("뉴진스").build()).id
    }
